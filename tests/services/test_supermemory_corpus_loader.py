...
class TestInitialization:
    ...
    @pytest.mark.asyncio
    async def test_init_without_api_key(self, loader):
    ...
    @pytest.mark.asyncio
    async def test_init_with_api_key(self, loader):
        """With API key, should initialize SupermemoryTools."""
        mock_tools = MagicMock()
        mock_tools.client = MagicMock()  # Not None
    ...
    @pytest.mark.asyncio
    async def test_init_with_import_error(self, loader):
        ...
        with patch.dict(os.environ, {"SUPERMEMORY_API_KEY": "test_key_123"}):
            with patch(
                ...
                # Manual call since the mock raises
                try:
                    await loader.initialize()
                except ImportError:
                    pass
                # Loader should not be available
                assert loader.available is False
...
class TestFetchMemories:
    ...
    @pytest.mark.asyncio
    async def test_fetch_when_unavailable(self, loader):
        """When not available, fetch should return empty list."""
        result = await loader.fetch_memories("test query")
        assert result == []

    @pytest.mark.asyncio
    async def test_fetch_returns_formatted_dicts(self, loader, mock_search_result):
        ...
        result = await loader.fetch_memories("JWT auth pattern", limit=10)
    ...
    @pytest.mark.asyncio
    async def test_fetch_deduplication(self, loader, mock_search_result):
        """Fetching same memories twice should deduplicate."""
        loader._available = True
        loader._supermemory = MagicMock()
        loader._supermemory.search = AsyncMock(return_value=mock_search_result)

        first = await loader.fetch_memories("query 1")
        assert len(first) == 3

        second = await loader.fetch_memories("query 2")
        assert len(second) == 0  # All IDs already loaded

        assert loader.memory_count == 3

    @pytest.mark.asyncio
    async def test_fetch_skips_empty_content(self, loader):
        ...
        results = await loader.fetch_memories("test")
        # Only good_1 should pass (empty_1 has empty string, empty_2 has None->falsy)
        assert len(results) == 1
        assert results[0]["memory_id"] == "good_1"

    @pytest.mark.asyncio
    async def test_fetch_handles_api_error(self, loader):
        """API errors should be caught and return empty list."""
        loader._available = True
        loader._supermemory = MagicMock()
        loader._supermemory.search = AsyncMock(side_effect=Exception("API timeout"))

        result = await loader.fetch_memories("test")
        assert result == []

    @pytest.mark.asyncio
    async def test_fetch_empty_result(self, loader, mock_empty_result):
        """Empty search results should return empty list."""
        loader._available = True
        loader._supermemory = MagicMock()
        loader._supermemory.search = AsyncMock(return_value=mock_empty_result)

        result = await loader.fetch_memories("no results query")
        assert result == []

    @pytest.mark.asyncio
    async def test_fetch_content_truncation(self, loader):
...
class TestFetchAsMCMPDocuments:
    ...
    @pytest.mark.asyncio
    async def test_mcmp_format(self, loader, mock_search_result):
        """MCMP documents should be formatted strings."""
        loader._available = True
        loader._supermemory = MagicMock()
        loader._supermemory.search = AsyncMock(return_value=mock_search_result)

        docs = await loader.fetch_as_mcmp_documents("test query")
    ...
    @pytest.mark.asyncio
    async def test_mcmp_empty_when_unavailable(self, loader):
        """Should return empty list when unavailable."""
        docs = await loader.fetch_as_mcmp_documents("test")
        assert docs == []
...
class TestFetchAsSearchResults:
    ...
    @pytest.mark.asyncio
    async def test_search_result_format(self, loader, mock_search_result):
        """Search results should have correct dict keys."""
        loader._available = True
        loader._supermemory = MagicMock()
        loader._supermemory.search = AsyncMock(return_value=mock_search_result)

        results = await loader.fetch_as_search_results("test query", limit=3)
    ...
    @pytest.mark.asyncio
    async def test_search_result_file_path_format(self, loader, mock_search_result):
        """file_path should be memory://category/id format."""
        loader._available = True
        loader._supermemory = MagicMock()
        loader._supermemory.search = AsyncMock(return_value=mock_search_result)

        results = await loader.fetch_as_search_results("test")
        assert results[0]["file_path"] == "memory://code_pattern/mem_001"
        assert results[1]["file_path"] == "memory://error_fix/mem_002"

    @pytest.mark.asyncio
    async def test_search_results_empty_when_unavailable(self, loader):
        """Should return empty list when unavailable."""
        results = await loader.fetch_as_search_results("test")
        assert results == []
...
class TestStorePattern:
    ...
    @pytest.mark.asyncio
    async def test_store_when_unavailable(self, loader):
        """Store should return False when unavailable."""
        result = await loader.store_pattern("content", "category")
        assert result is False

    @pytest.mark.asyncio
    async def test_store_success(self, loader, mock_store_result):
        ...
        result = await loader.store_pattern(
    ...
    @pytest.mark.asyncio
    async def test_store_handles_error(self, loader):
...
class TestUtility:
    ...
    @pytest.mark.asyncio
    async def test_close_with_supermemory(self, loader):
    ...
    @pytest.mark.asyncio
    async def test_close_without_supermemory(self, loader):
        """Close without client should not raise."""
        await loader.close()  # Should not raise
